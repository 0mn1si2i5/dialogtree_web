...
现在的布局是左-中-右，我想改一下：
新的布局是上-下，下再分左-中-右，上就是一个统一的 header，大概 50px 高，放置 logo 和一些按钮（比如切换语言、教程等）
下左就不要 logo 了（因为移动到了上），其余都一样。

---

round9.0

1-超时机制，前端有没有？很多时候误判了！

2- 每次进入一个先有绘画的时候默认是不会选择任何节点的，但这个使用不是体验很好，我希望每次自动选择整个会话下最新的对话（前端可以实现吗？需要后端吗？）

3- conversation title 字段有用了,title 为空的时候，每一个对话在tree节点中，那个对话框内显示的就是summary，但如果title不empty，那就显示title，
抬头在前端的入口就在对话炮的某一个角上，然后是一个小按钮，点了一下，它就会弹出一个小的对话框，就显示可以修改他的标题
另外我后端也实现了一个更新title的接口:
接口说明文档：

更新对话标题接口

接口地址： PUT /api/dialog/conversations/title
...
这个新的功能有几个问题？第一个就是我点了这个更改标题的按钮之后，它会跳出来一个茅台框，但是这个框里他有一个性状的标题，还有个新的标题，我不想要现在的标题你直接给我显示一个新的标题就行了，或者你把现在标题就显示成一个简单的文本，不要放在一个文字的那个输入栏里面。
第二个问题就是我点了更新之后这个模特框并不会自动消失，按理说点了之后它就应该自动关闭，另外呢，我点了手动关闭之后，他这个界面上的标题也就是这个对话框里面的标题并不会同步的更新，而是要等我刷新一次之后他才会跟着更新
还有一个问题就是我把原来的标题比如说是15个字，然后我改了之后改短了，那么他上下两边的这个线段他不会跟着边，因为我次数变短之后那个框的大小变少了，但那个线他就相当于选在半空中，他并没有连接到我的线
还有一个问题就是这么多选项，太密密麻麻了，能不能在我鼠标放到这个对话框？上面的时候再显示出这个更改标题的这个按钮平时他是不显示的。
最后还有个问题，不是关于这个更改标题的，而是评论的按钮的一个问题，就是当我想修改这个评论，比如说我就返回一个控制，相当于我删除吗？但他就会提示失败，我刚刚检查了一下后盾代码并没有控制这个内容不能为控，我估计是前段在哪儿控制了，你帮我排查一下，修改一下要能允许""。